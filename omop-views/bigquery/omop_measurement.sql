-- ─────────────────────────────────────────────────────────────────
-- omop_measurement — OMOP CDM 5.4
--
-- Source:  FHIR Observation (numeric) → forge-core `observation` table
-- Target:  OMOP MEASUREMENT
--
-- Only includes observations with a valid numeric value_quantity.
--
-- Placeholders: {project}, {dataset}
-- ─────────────────────────────────────────────────────────────────

CREATE OR REPLACE VIEW `{project}.{dataset}.omop_measurement` AS

SELECT
  ABS(FARM_FINGERPRINT(resource_id)) AS measurement_id,
  ABS(FARM_FINGERPRINT(patient_id)) AS person_id,
  0 AS measurement_concept_id,  -- requires vocabulary JOIN for LOINC → OMOP mapping
  ...
  44818702 AS measurement_type_concept_id,  -- From EHR
  0 AS operator_concept_id,
  SAFE_CAST(value_quantity AS FLOAT64) AS value_as_number,
  0 AS value_as_concept_id,
  0 AS unit_concept_id,  -- requires vocabulary JOIN for UCUM → OMOP mapping
