...
SELECT
  ABS(FARM_FINGERPRINT(CONCAT(patient_id, '|', encounter_id, '|', code))) AS measurement_id,
  ABS(FARM_FINGERPRINT(patient_id)) AS person_id,
  {{ resolve_concept('cm_meas', 0) }} AS measurement_concept_id,
  ...
  {{ resolve_concept('cm_unit', 0) }} AS unit_concept_id,
  ...
  {{ resolve_source_concept('sc_meas', 0) }} AS measurement_source_concept_id,
  value_unit AS unit_source_value,
  value_quantity AS value_source_value
FROM {{ ref('stg_observation') }} obs
{{ resolve_concept_join('cm_meas', 'obs.code', 'LOINC') }}
{{ resolve_concept_join('cm_unit', 'obs.value_ucum_code', 'UCUM') }}
{{ resolve_source_join('sc_meas', 'obs.code', 'LOINC') }}
